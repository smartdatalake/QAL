...
object density_cluster extends QueryEngine_Abs("DensityCluster") {
  ...
  val setup: Seq[(String, Long, Long)] = Seq(("4YearTo6Month", 4 * YEAR, 6 * MONTH), ("2WeekTo2Week", 2 * WEEK, 2 * WEEK))
  /*("1WeekTo1Week", 1 * OneWEEK, 1 * OneWEEK)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tuples = queryLog.filter("statement is not null and clientIP is not null")
      ...
      .map(x => (x.getAs[String](0), new Date(x.getAs[Int](1), x.getAs[Int](2), x.getAs[Int](3)
        , x.getAs[Int](4), x.getAs[Int](5), x.getAs[Int](6)).getTime / 1000, x.getAs[String](7))).collect()
    ...
    /*    for ((range, y, x) <- setup) {
          ...
          while (index < end) {
            ...
            val test = tuples.filter(r => index <= r._2 && r._2 < index + x)
            val processesTrain = new ListBuffer[ListBuffer[(String, Long, String)]]
            val processesTest = new ListBuffer[ListBuffer[(String, Long, String)]]
...
*/
    for ((range, y, x) <- setup) {
      ...
      while (index < end) {
        ...
        val resultPathTest = "/home/hamid/QAL/DensityCluster/" + tagTest
