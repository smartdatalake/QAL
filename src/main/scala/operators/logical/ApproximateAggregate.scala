...
case class ApproximateUniversalJoin(confidence:Double, error:Double, seed:Long, aggregateExpressions: Seq[AggregateExpression], child: LogicalPlan)
  extends UnaryNode {
  ...
  override def toString() = {
    "ApproximatedJoin(confidence " + confidence + " error " + error + " " + super.toString()
  }
}
