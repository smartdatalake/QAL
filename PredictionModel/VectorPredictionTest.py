...
modelDir = "models/"
setup = [ ("4YearTo1Month", 400, 32, 20),
         ("4YearTo2Week", 400, 32, 20), ("2YearTo2Year", 400, 32, 20), ("2YearTo1Year", 400, 32, 20),
         ("2YearTo6Month", 400, 32, 20), ("2YearTo3Month", 400, 32, 20), ("2YearTo1Month", 400, 32, 20),
         ("2YearTo2Week", 400, 32, 20), ("1YearTo1Year", 13, 32, 25), ("1YearTo6Month", 400, 32, 25),
         ("1YearTo3Month", 400, 32, 25), ("1YearTo1Month", 400, 32, 25), ("1YearTo2Week", 1000, 32, 25),
         ("6MonthTo6Month", 400, 16, 30), ("6MonthTo3Month", 400, 32, 25), ("6MonthTo1Month", 400, 32, 25),
         ("6MonthTo2Week", 400, 32, 25), ("3MonthTo3Month", 400, 32, 25), ("3MonthTo1Month", 400, 32, 25),
         ("3MonthTo2Week", 400, 32, 25), ("1MonthTo1Month", 400, 32, 30), ("1MonthTo2Week", 400, 32, 35),
         ("2WeekTo2Week", 400, 32, 50), ("1WeekTo1Week", 10000, 32, 70), ("1DayTo1Day", 10000, 32, 100)]
#
...
tagPostfix = sys.argv[2]  # "_gap1800_processMinLength3_maxQueryRepetition10000_featureMinFRQ1_reserveFeature15"
loss = sys.argv[3]
featureSize = 0
dropout = 0.3
...
for (tag, modelNumber, batchSize, nbEpoch) in setup:
    ...
    for index in range(1, modelNumber):
        ...
        for (windowSize, predictionSize) in [(10, 10)]:
            output = ""
            cp = time.time()
            ...
            modelName = tag + "_LSTM_M" + str(index) + "_" + str(windowSize) + "_" + str(predictionSize) + "_" + str(
                featureSize)
            model = load_model(modelDir + modelName + modelPath, modelDir + modelName + weightPath)
            model.compile(loss=loss, optimizer='adam',
                          metrics=[Tools.binary_fbeta, Tools.binary_p, Tools.binary_r, Tools.binary_acc1])
            cp = (time.time() - cp)
            t += cp
            ...
            out += modelName + "_" + str(round(cp)) + "_" + str(round(scores[1] * 100)) + "_" + str(
                ...
                round(scores[4] * 100)) + "_" + str(X_test.size/featureSize) + "\n"
    ...
    file_object = open('MLResultWeighted.txt', 'a')
